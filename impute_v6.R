...
if (newrun) {
  ...
  adataf<-create_agency_data()
  
  train_wk=9
  data_test1=create_lag_demands(test[Semana==10,],train[Semana>=2,],10,cdataf,pdataf,adataf,type='test1')
  data_test2=create_lag_demands(test[Semana==11,],train[Semana>=3,],11,cdataf,pdataf,adataf,type='test2')
  data_train=create_lag_demands(train[Semana==train_wk,],train[Semana>=(train_wk-8) & Semana<train_wk,],train_wk,cdataf,pdataf,adataf)
...
features2=names(data_train)[!(names(data_train) %in% c('Semana','Demanda_uni_equil','target','pred3_1','id'))]
...
options(digits=18)
write.csv(results,file='impute_v6_res1.csv',row.names=F)